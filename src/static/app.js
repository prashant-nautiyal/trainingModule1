        ...
        const participantsMarkup = details.participants.length
          ? `<div class="participants">
               ...
               <ul>${details.participants
                 .map(
                   (email) =>
                     `<li><span>${email}</span><button class="remove-participant" data-activity="${name}" data-email="${email}" aria-label="Remove ${email}">&times;</button></li>`
                 )
                 .join("")}</ul>
             </div>`
          : `<div class="participants empty">No participants have signed up yet.</div>`;
